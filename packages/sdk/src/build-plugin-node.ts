...
function adaptMountedRequest(request, env) {
  const incoming = env?.incoming;
  if (!incoming) return request;

  const normalizedPath = normalizeMountedPath(request, incoming);
  const url = new URL(normalizedPath, request.url);
  const headers = new Headers(request.headers);
  const shouldRewriteBody = request.method !== 'GET' && request.method !== 'HEAD' && hasParsedBody(incoming);
...
async function middlewareFetch(request, env) {
  return app.fetch(adaptMountedRequest(request, env), env);
}

const requestListener = getRequestListener(middlewareFetch);
